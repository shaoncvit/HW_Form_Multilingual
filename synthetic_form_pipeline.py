...
LAYOUT_REQUIRED_KEYS = {
    "membership": ["name", "dob", "gender", "phone", "email", "address", "city", "nationality", "membership_type", "occupation", "date", "signature"],
    "dense_office": ["name", "father_name", "mother_name", "dob", "gender", "phone", "email", "address", "pin_code", "city", "district", "state", "nationality", "occupation", "membership_type", "date", "signature"],
    "ta_reporting": ["name", "email", "subject", "date", "account_no", "membership_type", "remarks", "signature"],
    "university_table": ["exam_name", "date", "course", "remarks", "registration_no", "name", "roll_no", "phone", "email", "signature"],
    "tax_grid": ["name", "father_name", "dob", "pan", "phone", "address", "pin_code", "amount", "date", "signature"],
    "admission_marks": ["name", "father_name", "address", "phone", "gender", "dob", "course", "roll_no", "year", "board", "marks", "signature"],
    "prescription_sheet": ["name", "patient_id", "bed_no", "age", "gender", "date", "time", "doctor_name", "nurse_name", "bp", "hr", "rr", "spo2", "temperature", "grbs", "review_systems", "chief_complaints", "history_present_illness", "past_history", "procedure", "allergy", "diagnosis", "investigation_orders", "plan_of_care", "drug_name", "dose", "route", "frequency", "relation", "signature"],
}
...
class LayoutRenderer:
    ...
    def _draw_handwritten_in_region(self, draw: ImageDraw.ImageDraw, region: Tuple[int, int, int, int], text: str,
        ...
        # Mix neat writing and natural loose writing.
        # Neat mode keeps text properly inside the box; loose mode adds natural variation.
        mode = random.choices(["neat", "mid", "loose"], weights=[0.42, 0.36, 0.22], k=1)[0]

        if mode == "neat":
            left_pad = random.randint(4, max(6, min(14, region_w // 10)))
            x = x1 + left_pad
            if region_h > th + 6:
                y = y1 + max(0, (region_h - th) // 2) + random.randint(-2, 2)
            else:
                y = y1 + random.randint(0, 2)
            angle = random.uniform(angle_range[0] * 0.35, angle_range[1] * 0.35)
        else:
            ...
            if can_center and random.random() < center_bias:
                ...
                if mode == "loose" and random.random() < 0.55:
                    left_pad += random.randint(8, max(10, min(65, region_w // 4)))
                x = x1 + left_pad

            if loose_vertical and region_h > th + 8 and random.random() < (0.68 if mode == "mid" else 0.45):
                y = y1 + max(0, (region_h - th) // 2) + random.randint(-6, 6)
            else:
                max_pad = max(0, region_h - th - 2)
                y = y1 + random.randint(-2, max(0, min(max_pad, 10)))
            angle_scale = 0.65 if mode == "mid" else 1.0
            angle = random.uniform(angle_range[0] * angle_scale, angle_range[1] * angle_scale)
...
def build_keys_for_layout(family: str, layout_name: str, max_fields: Optional[int] = None) -> List[str]:
    base = build_keys_for_family(family, max_fields=None)
    extra = LAYOUT_REQUIRED_KEYS.get(layout_name, [])
    merged = []
    for k in base + extra:
        if k not in merged:
            merged.append(k)
    if max_fields is not None and max_fields > 0:
        sig = [k for k in merged if k == "signature"]
        non = [k for k in merged if k != "signature"][:max_fields]
        merged = non + sig
    return merged
...
def main() -> None:
    ...
    with manifest_path.open("a", encoding="utf-8") as mf:
        for local_i in range(args.num_samples):
            ...
            keys = build_keys_for_layout(family, layout_name, max_fields=args.max_fields if args.max_fields > 0 else None)
